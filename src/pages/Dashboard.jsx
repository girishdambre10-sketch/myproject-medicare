...
// ─── ICONS (emoji based) ─────────────────────────────────────────────────────
const ICONS = {
  dashboard: "📊", inventory: "📦", category: "🏷️", type: "💊",
  profile: "👤", logout: "🚪", search: "🔍", bell: "🔔",
  add: "➕", edit: "✏️", del: "🗑️", refresh: "🔄", eye: "👁️",
  alert: "⚠️", medicine: "🩺", cross: "✖️", check: "✔️",
  order: "🛒", sales: "💰", patient: "👥", low: "📉", up: "📈",
};
...
function Profile({ onLogout }) {
  const [profile, setProfile] = useState({ name: "Dr. Anjali Mehta", email: "admin@pharma.com", phone: "+91 98765 43210", role: "Admin Pharmacist", license: "PHARM-MH-20241", pharmacy: "PharmaRx Healthcare", address: "Mumbai, Maharashtra" });
  ...
  return (
    <div className="page">
      <div className="profile-header">
        <div className="avatar-lg">{profile.name.split(" ").map(n => n[0]).join("").slice(0, 2)}</div>
        <div>
          ...
          <div style={{ opacity: 0.6, fontSize: 13, marginTop: 4 }}>License: {profile.license}</div>
        </div>
      ...
      <div className="grid-2" style={{ gap: 20 }}>
        <div className="card">
          ...
          <div className="card-body">
            {[["Full Name", profile.name], ["Email", profile.email], ["Phone", profile.phone], ["Address", profile.address]].map(([label, val]) => (
        ...
        <div className="card">
          ...
          <div className="card-body">
            {[["Role", profile.role], ["License No.", profile.license], ["Pharmacy", profile.pharmacy]].map(([label, val]) => (
              <div key={label} style={{ display: "flex", padding: "10px 0", borderBottom: "1px solid var(--border)" }}>
                <span style={{ color: "var(--text-muted)", fontSize: 13, width: 120, flexShrink: 0 }}>{label}</span>
                <span style={{ fontSize: 13, fontWeight: 500 }}>{val}</span>
              </div>
      ...
      {edit && (
        <Modal title="Edit Profile" onClose={() => setEdit(false)}
          footer={<><button className="btn btn-outline" onClick={() => setEdit(false)}>Cancel</button><button className="btn btn-primary" onClick={handleSave}>Save Changes</button></>}>
          <div className="form-row">
          ...
          <div className="form-row">
            ...
            <div className="form-group"><label className="form-label">Role</label><input className="form-control" value={form.role} onChange={e => setForm({ ...form, role: e.target.value })} /></div>
          </div>
          <div className="form-row">
            <div className="form-group"><label className="form-label">License No.</label><input className="form-control" value={form.license} onChange={e => setForm({ ...form, license: e.target.value })} /></div>
            <div className="form-group"><label className="form-label">Pharmacy Name</label><input className="form-control" value={form.pharmacy} onChange={e => setForm({ ...form, pharmacy: e.target.value })} /></div>
          </div>
        ...
        <div className="sidebar">
          ...
          <div className="sidebar-footer">
            <div className="sidebar-profile" onClick={() => setPage("profile")}>
              ...
              <div>
                <div className="profile-name">Dr. Anjali Mehta</div>
